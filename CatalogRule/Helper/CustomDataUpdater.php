...
use MMT\CatalogRule\Api\Data\RuleInterface;
use MMT\HomeSlider\Api\Data\HomeSliderInterface;
use MMT\CatalogRule\Helper\HomeSliderGenerator;
use MMT\HomeSlider\Model\HomeSliderRepository;
use MMT\HomeSlider\Model\HomeSlider;
...
class CustomDataUpdater extends AbstractHelper
{
     ...
     * @param string $url
     */
    public function updateData($id, $type, $categoryId, $websiteId, $url)
    {

        if ($type === 'delete') {
            $this->homeSliderGenerator->deleteSliderBlock(self::BLOCK_IDENTIFIER, self::HOME_SLIDER, $id, $websiteId, 0, self::PROMO_TOTAL_DIV_COUNT_TO_REMOVE);
            $this->homeSliderGenerator->deleteSliderBlock(self::BLOCK_IDENTIFIER, self::HOME_SLIDER_ONE, $id, $websiteId, 0, self::PROMO_TOTAL_DIV_COUNT_TO_REMOVE);
            $this->homeSliderGenerator->deleteSliderBlock(self::BLOCK_IDENTIFIER, self::HOME_SLIDER_MM, $id, $websiteId, 0, self::PROMO_TOTAL_DIV_COUNT_TO_REMOVE);
            $this->homeSliderGenerator->deleteSliderBlock(self::BLOCK_IDENTIFIER, self::HOME_SLIDER_ONE_MM, $id, $websiteId, 0, self::PROMO_TOTAL_DIV_COUNT_TO_REMOVE);
            ...
            if ($homeSliderRule->getIsActive() == self::INACTIVE_STATUS_CODE) {
                ...
                $this->updateRuleData($homeSliderRule, $categoryId, $url);
            }
        }
    }
     ...
     * @param \MMT\HomeSlider\Api\Data\HomeSliderInterface $catalogRule
     * @param int $categoryId
     * @param string $customUrl
     */
    private function updateRuleData($homeSliderRule, $categoryId, $customUrl)
    {
        ...
        $mobileImgUrl = '';
        $discount = '';
        if ($categoryId && $categoryId > 0 || $categoryId && $categoryId > 0 && $customUrl && $customUrl != "#") {

            $identifier = 'hm__000__' . $this->homeSliderModel->getHomesliderId();
            $this->homeSliderModel->deletPage($identifier);
        ...
        if ($homeSliderRule->getIsHomeSlider() == self::ACTIVE_STATUS_CODE) {
            $this->homeSliderGenerator->generateAndUpdateHomeSlider($homeSliderRule->getWebsiteId(), self::BLOCK_IDENTIFIER, self::HOME_SLIDER, $homeSliderRule->getHomesliderId(), $img, $mobileImgUrl, $url, $categoryId);
            $this->homeSliderGenerator->generateAndUpdateHomeSlider($homeSliderRule->getWebsiteId(), self::BLOCK_IDENTIFIER, self::HOME_SLIDER_MM, $homeSliderRule->getHomesliderId(), $img, $mobileImgUrl, $url, $categoryId);
            ...
            $this->homeSliderGenerator->deleteSliderBlock(self::BLOCK_IDENTIFIER, self::HOME_SLIDER, $homeSliderRule->getHomesliderId(), $homeSliderRule->getWebsiteId(), 0, self::PROMO_TOTAL_DIV_COUNT_TO_REMOVE);
            $this->homeSliderGenerator->deleteSliderBlock(self::BLOCK_IDENTIFIER, self::HOME_SLIDER_MM, $homeSliderRule->getHomesliderId(), $homeSliderRule->getWebsiteId(), 0, self::PROMO_TOTAL_DIV_COUNT_TO_REMOVE);
        }

        if ($homeSliderRule->getIsHomeSliderOne() == self::ACTIVE_STATUS_CODE) {
            $this->homeSliderGenerator->generateAndUpdateHomeSlider($homeSliderRule->getWebsiteId(), self::BLOCK_IDENTIFIER, self::HOME_SLIDER_ONE, $homeSliderRule->getHomesliderId(), $img, $mobileImgUrl, $url, $categoryId);
            $this->homeSliderGenerator->generateAndUpdateHomeSlider($homeSliderRule->getWebsiteId(), self::BLOCK_IDENTIFIER, self::HOME_SLIDER_ONE_MM, $homeSliderRule->getHomesliderId(), $img, $mobileImgUrl, $url, $categoryId);
            ...
            $this->homeSliderGenerator->deleteSliderBlock(self::BLOCK_IDENTIFIER, self::HOME_SLIDER_ONE, $homeSliderRule->getHomesliderId(), $homeSliderRule->getWebsiteId(), 0, self::PROMO_TOTAL_DIV_COUNT_TO_REMOVE);
            $this->homeSliderGenerator->deleteSliderBlock(self::BLOCK_IDENTIFIER, self::HOME_SLIDER_ONE_MM, $homeSliderRule->getHomesliderId(), $homeSliderRule->getWebsiteId(), 0, self::PROMO_TOTAL_DIV_COUNT_TO_REMOVE);
        }
