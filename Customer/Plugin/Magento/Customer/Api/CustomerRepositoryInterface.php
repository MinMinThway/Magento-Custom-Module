...
namespace MMT\Customer\Plugin\Magento\Customer\Api;

use Magento\Framework\App\ResourceConnection;
use Magento\Framework\Exception\LocalizedException;
use Magento\Framework\Registry;
use MMT\Customer\Helper\CustomerHelper;
use MMT\Customer\Model\Api\CustomAccountManagement;
...
class CustomerRepositoryInterface
{
    ...
    public function beforeSave(
        ...
        $profileImg = $customer->getExtensionAttributes()->getProfileImg() ;      
        if($customer->getId()){
            $this->saveProfileImg($customer->getId(), $profileImg);
        }  
     ...
     * @param string $value
     */
    private function saveProfileImg($customerId, $value)
    {
        $connection = $this->resourceConnection->getConnection();
                $tableName = $connection->getTableName('customer_entity');
                $connection->update($tableName,
    ...
    /**
     * Plugin after getById customer that obtains customer profile_img for given customer.
     *
     ...
     */
    public function afterGetById(\Magento\Customer\Api\CustomerRepositoryInterface $subject, $customer)
    {
        $extensionAttributes = $customer->getExtensionAttributes();
        if ($extensionAttributes === null || $extensionAttributes->getProfileImg() === null) {
            $connection = $this->resourceConnection->getConnection();
            $tableName = $this->resourceConnection->getTableName('customer_entity');
            $sql = $connection->select()
                ->from($tableName, 'profile_img')
                ->where('entity_id = ? ', $customer->getId());
            $result = $connection->fetchOne($sql);
            $this->addProfileImgExtensionAttribute($customer, $result);
        }
    ...
    /**
     * Plugin after create customer that updates customer profile image that may have existed.
     *
     * @param \Magento\Customer\Api\CustomerRepositoryInterface $subject
     * @param CustomerInterface $customer
     * @return CustomerInterface $customer
    ...
    /**
     * add profile image extension attribute
     *
     * @param $customer
     * @param string $img
     */
    private function addProfileImgExtensionAttribute( $customer, $img): void
    {       
        $extensionAttributes = $customer->getExtensionAttributes();
        ...
        $extensionAttributes->setProfileImg($img);
    }
}
