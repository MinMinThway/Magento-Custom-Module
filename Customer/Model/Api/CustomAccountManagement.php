...
namespace MMT\Customer\Model\Api;
...
use MMT\Customer\Api\CustomAccountManagementInterface;
use MMT\Customer\Helper\AttributeChecker;
use MMT\Customer\Helper\CustomerHelper;
use MMT\Customer\Helper\SMSSender;
use MMT\Customer\Model\AccountConfigFactory;
use MMT\Customer\Model\CustomerEmailNotification;
use MMT\Product\Api\Data\StatusShowInterfaceFactory;
use Zend_Db_Expr;
...
class CustomAccountManagement extends AccountManagement implements CustomAccountManagementInterface
{
    ...
    /**
     * @var \MMT\Customer\Helper\AttributeChecker
     */
    ...
    public function __construct(
        ...
        \MMT\Customer\Helper\AttributeChecker $attributeChecker,
    ...
    {
        $array = explode(".", $files->getName());
        $image_name = $array[0];
        $extension = pathinfo($files->getName())['extension'];
        ...
        try {
            if (!in_array($extension, $allowedExtension)) {
                $errorMessage[] = "Invalid file format (Files allowed: png, jpg, jpeg, gif).";
                return $this->showStatus($status, $errorMessage, $successMessage);
            }
            $image = $image_name . "_" . time() . "." . $extension;

            $destinationFolder = 'profileUpload/customer';
            $imageContent = $this->imageContentFactory->create();
            $imageContent->setBase64EncodedData($files->getBase64EncodedData());
            $imageContent->setType($files->getType());
            $imageContent->setName($image);
            $filename = $this->imageProcessor->processImageContent($destinationFolder, $imageContent);

            $status = true;
            $successMessage[] = $this->storeManagerInterface->getStore()->getBaseUrl() . 'media/profileUpload/customer/' . $filename;
            return $this->showStatus($status, $errorMessage, $successMessage);
    ...
    public function howToCreateAccountVideo($type){
        $filenameone = 'how_to_create_account_and_sign_in.mp4';
        $filenametwo = 'how_to_buy.mp4';
        
        $status = false;
        if($type == 0){
            $successMessage[] = $this->storeManagerInterface->getStore()->getBaseUrl() . 'media/wysiwyg/videos/' . $filenameone;
            $status = true;
        } elseif($type == 1){
            $successMessage[] = $this->storeManagerInterface->getStore()->getBaseUrl() . 'media/wysiwyg/videos/' . $filenametwo;
            $status = true;
        }
        return $this->showStatus($status, [], $successMessage);
    }
