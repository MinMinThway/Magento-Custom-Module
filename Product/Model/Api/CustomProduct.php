...
// use MMT\Discount\Helper\DiscountHelper;
use MMT\Product\Api\CustomProductInterface;
use MMT\Product\Api\Data\CustomProductSearchResultsInterfaceFactory;
use MMT\Product\Helper\BundleProductHelper;
use MMT\Product\Helper\ProductHelper;
use MMT\Product\Helper\ShoppingOptionsFilter;
use MMT\Product\Model\CustomProductFactory;
use MMT\Product\Model\DiscountLabelFactory;
use Magento\Framework\Stdlib\DateTime\TimezoneInterface;
use Magento\Store\Model\ScopeInterface;
use Magento\Catalog\Helper\Image as ImageHelper;
use MMT\Product\Helper\SimpleProductHelper;
...
class CustomProduct extends ProductRepository implements CustomProductInterface
{
    ...
    /**
     * @var \MMT\Product\Api\Data\customProductSearchResultsInterface
     */
    ...
    /**
     * @var \MMT\Product\Model\GroupProductFactory
     */
    ...
    /**
     * @var SimpleProductHelper 
     */
    private $simpleProductHelper;
    ...
    public function __construct(
        ...
        \MMT\Product\Model\GroupProductFactory $groupProductFactory,
        ...
        SimpleProductHelper $simpleProductHelper,
        ...
        $this->simpleProductHelper = $simpleProductHelper;
        ...
        if (($product->getSku())) {
            ...
            if(isset($attributeArray)){
                foreach ($attributeArray as $attributeList) {
                    ...
                    ->where('attribute_id IN (?)', $attributeListAttributeId);    
                    $result = $connection->fetchAll($query);    
                    foreach ($result as $row) {
                        $configAttributeCode[] = $row['attribute_code']; 
                    }             
                }
            }
            ...
            if ($product->getMediaGalleryEntries()) {
                ...
                foreach ($product->getMediaGalleryEntries() as $gallery) {
                    $data = $gallery;
                    if ($data->getFile()) {
                        $data->setFile($this->imageHelper->init($product, 'product_page_image_small')
                            ->setImageFile($product->getSmallImage())->resize(512)->getUrl());
            ...
            if ($product->getExtensionAttributes()->getConfigurableProductLinks()) {
                ...
                $childProduct = $this->simpleProductHelper->getChildProductList($productIds,$configAttributeCode);                
                $product->setConfigurableProductList($childProduct);
            }           
                ...
                foreach ($wishListProduct as $item) {
                    ...
                    if ($childProduct) {
                        foreach ($childProduct as $child) {
                            if ($item['selected_product_id'] == $child->getId()) {
                                $this->setWishlistStatusForItems($product, true, $wishlistItemId, $wishlistQty, $item['selected_product_id']);
                            }
                        }
                    }

                    if ($item['selected_product_id'] == $id) {
                        $this->setWishlistStatusForItems($product, true, $wishlistItemId, $wishlistQty, $item['selected_product_id']);
                    }
                }
            }
    ...
    public function getCustomProductDetailsList(SearchCriteriaInterface $searchCriteria, $customSort = false, $sortQuery = '')
    {
        ...
        $scopeTz = new \DateTimeZone(
            $this->timezoneInterface->getConfigTimezone(ScopeInterface::SCOPE_WEBSITE, $this->storeManager->getStore()->getWebsiteId())
        );
        ...
        (mit_discount_label.to_time = 0 or mit_discount_label.to_time > ' . $date . ') ',
            array(
                'catalogrule_label' => 'discount_label',
                'catalogrule_width' => 'width',
                'catalogrule_height' => 'height',
                'catalogrule_sortorder' => 'sort_order',
                'catalogrule_labelcolor' => 'discount_label_color',
                'catalogrule_labelstyle' => 'discount_label_style'
            )
        );
        ...
        (mit_salesrule_label_product.to_time = 0 or mit_salesrule_label_product.to_time > ' . $date . ' ) ',
            array(
                'salesrule_label' => 'discount_label',
                'salesrule_width' => 'width',
                'salesrule_height' => 'height',
                'salesrule_sortorder' => 'sort_order',
                'salesrule_labelcolor' => 'discount_label_color',
                'salesrule_labelstyle' => 'discount_label_style'
            )
        );


        // $collection->getSelect()->joinLeft(
        //     'mit_discount_label_image',
        //     'mit_discount_label.discount_img_id = mit_discount_label_image.label_image_id',
        //     array('catalogrule_imgpath' => 'img_path', 'catalogrule_cssclass' => 'main_css_class', 'catalogrule_subcssclass' => 'sub_css_class')
        // );

        // $collection->getSelect()->joinLeft(
        //     'mit_discount_label_image',
        //     'mit_salesrule_label_product.discount_image_id = mit_discount_label_image.label_image_id',
        //     array('salesrule_imgpath' => 'img_path', 'salesrule_cssclass' => 'main_css_class', 'salesrule_subcssclass' => 'sub_css_class')
        // );
        ...
        foreach ($collection->getItems() as $item) {
            $this->retrieveAndSetDiscountLabelList($item);
            ...
            if ($item->getCustomAttributes()) {
                foreach ($item->getCustomAttributes() as $customAttribute) {
                    if (in_array($customAttribute->getAttributeCode(), ['image', 'thumbnail', 'small_image', 'swatch_image'])) {
                        //$customAttribute->setValue($baseUrl . 'media/catalog/product' . $customAttribute->getValue());
                        $customAttribute->setValue($this->imageHelper->init($item, 'product_page_image_small')
            ...
            if ($customerId > 0) {
                ...
                foreach ($wishListProduct as $product) {
                    ...
                    if ($item->getTypeId() == "configurable") {
                        $childProduct = $item->getTypeInstance()->getUsedProducts($item);
                        foreach ($childProduct as $child) {
                            if ($product['selected_product_id'] == $child->getId()) {
                                $this->setWishlistStatusForItems($item, true, $wishlistItemId, $wishlistQty, $product['product_id']);
                            }
                        }
                    }
                    if ($product['selected_product_id'] == $item->getId()) {
                        $this->setWishlistStatusForItems($item, true, $wishlistItemId, $wishlistQty, $product['selected_product_id']);
                    }
                }
            }
    ...
    public function getCustomRegularPrice($product)
    {
        ...
        $avgRating = $this->getRatingSummary($product);
        $product->setAverageRating($avgRating ? $avgRating : '0');
    ...
    /**
     * get product url
     * @param \MMT\Product\Model\CustomProduct $product
     ...
     * @param \MMT\Product\Api\Data\CustomProductManagementInterface $product
     * @return $this
     */
    private function retrieveAndSetDiscountLabelList($product)
    {
        $type = 0;
        if (isset($product['salesrule_sortorder']) && isset($product['catalogrule_sortorder'])) {
            $type = 1;
            if ($product['salesrule_sortorder'] > $product['catalogrule_sortorder']) {
                $type = 2;
            }
        } else if ($product['catalogrule_sortorder']) {
            $type = 1;
        } else if ($product['salesrule_sortorder']) {
            $type = 2;
        }
        ...
        if ($type > 0) {
            ...
            if ($type == 1) {
                $discount->setLabel(($product['catalogrule_label']) ? $product['catalogrule_label'] : '');
                $discount->setImgPath(($product['catalogrule_imgpath']) ? $product['catalogrule_imgpath'] : '');
                $discount->setMainClass(($product['catalogrule_cssclass']) ? $product['catalogrule_cssclass'] : '');
                $discount->setSubClass(($product['catalogrule_subcssclass']) ? $product['catalogrule_subcssclass'] : '');
                $discount->setStyle('');
                $discount->setLabelStyle(($product['catalogrule_labelstyle']) ? $product['catalogrule_labelstyle'] : '');
            } else {
                $discount->setLabel(($product['salesrule_label']) ? $product['salesrule_label'] : '');
                $discount->setImgPath(($product['salesrule_imgpath']) ? $product['salesrule_imgpath'] : '');
                $discount->setMainClass(($product['salesrule_cssclass']) ? $product['salesrule_cssclass'] : '');
                $discount->setSubClass(($product['salesrule_subcssclass']) ? $product['salesrule_subcssclass'] : '');
                $discount->setStyle('');
                $discount->setLabelStyle(($product['salesrule_labelstyle']) ? $product['salesrule_labelstyle'] : '');
            }
    ...
    /**
     * set wishlist status to items
     * @param mixed $item
     * @return void
     */
    private function setWishlistStatusForItems($item, $status, $wishlistItemId, $wishlistQty, $selectedProductId)
    {
        $item->setIsWishList($status);
