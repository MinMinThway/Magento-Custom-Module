...
use MMT\Product\Api\CustomLayerNavigationInterface;
...
class CustomLayerNavigation implements CustomLayerNavigationInterface
{
    ...
    /**
     * @var \MMT\Product\Model\Layer\FilterListFactory
     */
    ...
    public function __construct(
        ...
        \MMT\Product\Model\Layer\FilterListFactory $filterListFactory,
    ...
    public function getLayerNavigationByCategoryIdAndActivFilters($categoryId, array $items)
    {
        ...
        $activeFilterArr = [];
        if (count($items) > 0) {
            /** @var \MMT\Product\Model\CustomKeyVal $item */
    ...
    private function generateLayerNavigationFilter($filters)
    {
        ...
        foreach ($filters as $filter) {
            ...
            foreach ($items as $item) {
                ...
                $filterValues[$j]['value']   = strval($item->getValue());
